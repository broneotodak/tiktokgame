...
function buildCommentaryPrompt(eventType, eventData, recentContext, liveContext, viewerProfile, conversationLog) {
  ...
  switch (eventType) {
    ...
    case 'mystic_question':
      return `${contextStr}\nViewer bernama "${eventData.name}" — PANGGIL DIA "${eventData.name}"! Kartu: "${eventData.tarotCard || 'The Hermit'}", posisi ${eventData.tarotPosition || 'Tegak'}. Makna: "${eventData.tarotMeaning || ''}". Dia nanya: '${eventData.question}'. Jawab berdasarkan kartu DAN posisinya — ${eventData.tarotPosition?.includes('Terbalik') ? 'terbalik = jawabannya ada tantangan/pelajaran' : 'tegak = jawaban positif'}. 20-30 kata.`;
    ...
    case 'mystic_idle_tarot':
      return `${contextStr}\nAda ${eventData.viewerCount} viewers. Eyang ambil satu kartu tarot dari dek: "${eventData.tarotCard}". Posisi: ${eventData.tarotPosition}. Makna: "${eventData.tarotMeaning}". ${eventData.tarotPosition?.includes('Terbalik') ? `Kalau tegak maknanya: "${eventData.tarotMeaningOpposite}".` : `Kalau terbalik maknanya: "${eventData.tarotMeaningOpposite}".`} Jelaskan kartu ini ke viewers — apa artinya kalau muncul dalam reading, bedanya tegak vs terbalik. Buat mistis dan mendidik. Ajak mereka kirim gift biar dapat kartu sendiri. 30-45 kata.`;
