...
function buildCommentaryPrompt(eventType, eventData, recentContext) {
  ...
  switch (eventType) {
    ...
    case 'mystic_gift_reading':
      return `${contextStr}\nViewer bernama "${eventData.name}" — PANGGIL DIA "${eventData.name}"! Dia kasih ${eventData.diamonds} diamonds! Kartu tarot: "${eventData.tarotCard || 'Wheel of Fortune'}".${eventData.zodiac ? ` Zodiak: ${eventData.zodiac}.` : ''}${eventData.focusTopic ? ` Dia minta fokus ke: ${eventData.focusTopic}. Pertanyaan: "${eventData.question || ''}".` : ''} Buat reading UNIK — hubungkan kartu dengan zodiak.${eventData.focusTopic ? ` Fokuskan pada ${eventData.focusTopic}.` : ' Prediksi: cinta, karir/bisnis.'} Angka bertuah (3 angka), warna bertuah. Apresiasi gift-nya. 40-60 kata.`;
